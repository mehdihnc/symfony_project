...
function Board(selector, where) {
  ...
  this.addTree = function (index, x, y, z) {
    var tree =
      '<div class="tree"><div class="branch"></div><div class="branch"></div><div class="trunk"></div><div class="shadow"></div></div>';
...
function Node(cell) {
  ...
  this.render = function () {
    ...
    if (!cell.isTaken()) {
      cell
        .markTaken()
        .append(
          "<div class='node'></div><div class='east edge'></div><div class='south edge'></div>"
        );
    }
  ...
  this.branch = function (directions, probability) {
    ...
    while (
      (curr_cell.open().length < num && !curr_cell.dead_end()) ||
      (rand_num <= probability && !curr_cell.dead_end())
    ) {
  ...
  this.switchBack = function (next) {
    ...
    if (next == avoid) {
      ...
      // add the timer
      this.after(
        '<div class="js-inkdot inkdot non blacknwhite" style="animation-duration: ' +
          time_limit +
          's;">↻</div>'
      );
  ...
  this.toJoint = function (start, end) {
    ...
    if (start.slice(-2) != end.slice(-2)) {
      cell.append(
        "<div class='planks'><div class='plank'></div><div class='plank'></div><div class='plank'></div></div>"
      );
    }
...
function House(node, direction, color) {
  ...
  var house =
    ...
    " " +
    color +
    '"><div class="front"><div class="tunnel"></div></div><div class="side">' +
    node.id() +
    '</div><div class="roof"></div></div>';
...
function Train(destination, speed) {
  var train =
    "<div class='train'><div class='pilot'><div class='destination js-destination'>" +
    destination +
    "</div></div><div class='stack'></div><div class='engine'></div><div class='wheels'><div class='wheel'><div class='spoke'></div><div class='spoke'></div><div class='spoke'></div><div class='spoke'></div></div><div class='wheel'><div class='spoke'></div><div class='spoke'></div><div class='spoke'></div><div class='spoke'></div></div><div class='wheel'><div class='spoke'></div><div class='spoke'></div><div class='spoke'></div><div class='spoke'></div></div></div></div>";
  var station = null;
  var status = "";
  var max_time = 1000;
  ...
  this.advance = function (node) {
    ...
    var j_train = $(train)
      .addClass("to_" + direction)
      .css("animation-name", "to_" + direction)
      .css("animation-duration", speed * 0.8 + "s");
    ...
    if (op_node.isExtendedLeaf()) {
      ...
      if (next_node.isJustLeaf() || next_node.isDeadEnd()) {
        if (
          (next_node.isJustLeaf() && next_cell.hasClass("west")) ||
          next_node.isDeadEnd()
        ) {
          max_time = 370;
        } else {
        ...
        setTimeout(function () {
          ...
          j_train
            .css("animation-name", direction + "_to_" + to)
            .css("animation-duration", speed * 0.2 + "s");
        }, speed * 800);
      }
    }
...
$(document).on("click", ".js-instructions", function () {
  html =
    "<div class='instructions-box'><div class='instructions'> " +
    "<h2>INSTRUCTIONS</h2>" +
...
$(document).on(
  "click",
  ".js-close-overlay, .overlay, .js-continue",
  function () {
...
function addOverlay(html, selector) {
  ...
  $("body").append(
    "<div class='overlay'></div><div class='" +
      selector +
      " js-overlay-box overlay-box'><p class='js-close-overlay'><span class='close'>X</span></p></div>"
  );
...
function Game(difficulty) {
  ...
  this.finished = function () {
    ...
    if (accuracy == 100) {
      headline =
        '<span style="color: rgb(160, 212, 104);">G</span><span style="color: rgb(79, 193, 233);">R</span><span style="color: #ED5565;">E</span><span style="color: rgb(255, 206, 84);">A</span><span style="color: rgb(140, 193, 82);">T</span>&nbsp;<span style="color: rgb(252, 110, 81);">J</span><span style="color: rgb(93, 156, 236);">O</span><span style="color: rgb(236, 135, 192);">B</span>';
      message = "You got all the trains to the right station!";
      success = true;
    } else if (accuracy >= 80) {
      headline =
        '<span style="color: rgb(252, 110, 81);">G</span><span style="color: #ED5565;">O</span><span style="color: rgb(140, 193, 82);">O</span><span style="color: rgb(93, 156, 236);">D</span>&nbsp;<span style="color: #967ADC;">J</span><span style="color: rgb(55, 188, 155);">O</span><span style="color: rgb(74, 137, 220);">B</span>';
      message = "You're ready for the next level.";
      success = true;
    } else {
      headline =
        "<span>Y</span><span>I</span><span>K</span><span>E</span><span>S</span>";
    ...
    html =
      ...
      headline +
      "</div><div class='accuracy'>Accuracy " +
      accuracy +
      "%</div><p class='message'>" +
      message +
      "</p><button class='js-continue centered'>CONTINUE</button></div>";
  ...
  this.animateScore = function (points, cell) {
    ...
    cell.append(
      "<div class='chewy rise " + puff + " " + color + "'>" + points + "</div>"
    );
  };
  ...
  this.draw = function () {
    ...
    tree
      ...
      .cell()
      .append(
        '<div class="js-settings"><button class="js-start start">START</button><button class="js-change_map change_map">CHANGE MAP</button><div class="difficulty"><button class="js-easier">-</button><div class="level">LEVEL <span class="js-level">' +
          difficulty +
          '<span></div><button class="js-harder">+</button></div></div>'
      );
    var death_count = 0;
  ...
  this.decorate = function (num) {
    ...
    for (var i = 0; i < num; i++) {
      ...
      if (
        cell.children(".house.south").length > 0 ||
        cell.children(".house.west").length > 0
      ) {
        maxY = 22;
      }
...
function Death(node) {
  var death =
    '<div class="death"><div class="head"><div class="skull"><div class="eyes"><div class="eye eye-left"></div><div class="eye eye-right"></div></div></div></div><div class="body"></div><div class="legs"></div></div>';
...
$(document).on("click", ".js-tutorial", function () {
  ...
  $(".root")
    .parent()
    .append(
      '<div class="tutorial_message">Guide the trains to the right stations.</div>'
    );

  setTimeout(function () {
    $(".tutorial_message")
      .css("width", "4em")
      .html("Trains will start here. ↓");
  ...
  setTimeout(function () {
    ...
    $(".root")
      .parent()
      .append(
        '<div class="tutorial_message">Click on the joints to switch the direction of the train tracks.</div>'
      );
  }, 19000);
  ...
  setTimeout(function () {
    ...
    $(".tutorial_message")
      .html(
        "Good luck! As you progress the map will expand and the trains will quicken."
      )
      .css("width", "11em");
  }, 26000);
